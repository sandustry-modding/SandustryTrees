...
test("collapseIfDetached drops a trunk when the chopped cell still reads as wood", () => {
  ...
  for (let y = 8; y <= 12; y += 1) {
    cells.set(`${trunkX},${y}`, { terrain: types.oakWood });
  }
...
test("collapseIfDetached clears a multi-cell trunk in one harvest", () => {
  ...
  const oakDropped = writes.filter((entry) => entry.startsWith("element:")).length;
  assert.ok(oakDropped >= 3, `detached trunk should convert wood cells, got ${oakDropped}`);
  assert.ok(
    writes.some((entry) => entry.startsWith(`terrain:${trunkX},${choppedY - 1}`)),
    `expected collapse to convert trunk above the chop, got ${writes.join("; ")}`,
  );
...
test("collapseIfDetached does not convert overlapping wood below the chop", () => {
  ...
  for (let y = 8; y <= 14; y += 1) {
    cells.set(`${trunkX + 1},${y}`, { terrain: types.oakWood });
  }
  ...
  assert.equal(cells.get(`${trunkX},${choppedY + 1}`)?.terrain, types.oakWood);
  assert.equal(cells.get(`${trunkX + 1},${13}`)?.terrain, types.oakWood);
  assert.equal(
    writes.some((entry) => entry.startsWith(`terrain:${trunkX},${choppedY + 1}`)),
    false,
  );
...
test("collapseIfDetached does not convert wood outside the collapse radius", () => {
  ...
  assert.equal(cells.get(`${trunkX},${farY}`)?.terrain, types.oakWood);
  assert.equal(
