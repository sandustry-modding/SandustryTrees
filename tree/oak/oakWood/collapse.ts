...
/** Run work without harvest collapse (oak-wood creates during growth). */
...
function isTrunkCell(
  ...
  if (api.terrains.getTypeAtCell(cellX, cellY) === types.oakWood) return true;
  return api.elements.isTypeAtCell(cellX, cellY, types.oakShoot);
}
...
function isTreeCell(
  ...
  return api.elements.isTypeAtCell(cellX, cellY, types.oakLeaf);
}
...
function dropCell(api: TreeApi, types: HarvestTypes, cell: Cell, writer: DropWriter): void {
  ...
  if (api.terrains.getTypeAtCell(cell.x, cell.y) === types.oakWood) {
    writer.terrains.removeAtCell(cell.x, cell.y);
    writer.elements.createAtCell(cell.x, cell.y, types.wood, fall);
  } else if (api.elements.isTypeAtCell(cell.x, cell.y, types.oakLeaf)) {
    writer.elements.replaceAtCell(cell.x, cell.y, types.compost, fall);
  } else if (api.elements.isTypeAtCell(cell.x, cell.y, types.oakShoot)) {
