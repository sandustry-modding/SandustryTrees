...
test("one seed is 48 cells tall with the old cone", () => {
  assert.equal(targetHeightForSeedCount(1), TRUNK_HEIGHT);
  assert.equal(halfWidthForSeedCount(1), TRUNK_HALF_WIDTH);
  assert.equal(canopyRowsForHalfWidth(1), CANOPY_ROWS);
  assert.equal(canopyMaxHalfForHalfWidth(1), CANOPY_MAX_HALF_WIDTH);
});
...
test("trunk width follows seed count until the width cap", () => {
  assert.equal(halfWidthForSeedCount(1), 1);
