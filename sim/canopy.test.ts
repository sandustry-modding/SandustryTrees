...
test("a wider trunk uses a wider needle cone", () => {
  const wide = canopyMaxHalfForHeight(TRUNK_HEIGHT, TRUNK_HEIGHT, 2);
  assert.ok(wide > CANOPY_MAX_HALF_WIDTH);
  const cells = canopyRowCells(10, 100, CANOPY_ROWS - 1, TRUNK_HEIGHT, TRUNK_HEIGHT, 2);
  for (const cell of cells) {
    const trunkTopY = canopyTreeTopY(100);
    if (cell.y >= trunkTopY) {
      assert.ok(Math.abs(cell.x - 10) > 2);
    }
  }
});
